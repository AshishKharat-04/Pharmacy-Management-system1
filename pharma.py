...
class PharmacyManagementSystem:
    def __init__(self,root):
        ...
        img1=Image.open("E:\Pharmacy management System\logos.jpg")
        ...
        search_combo["values"]=("Medname","Lot")
        ...
        conn=mysql.connector.connect(host="localhost",username="root",password="PHW#84#jeor",database="mydata")
        ...
        lblTypeofMedicine=Label(DataFrameLeft,font=("arial",13,"bold"),text="TypeofMedicine",padx=2,pady=6)
        ...
        lblMedicineName=Label(DataFrameLeft,font=("arial",13,"bold"),text="Medicine Name",padx=2,pady=6)
        ...
        lblPrecWarning=Label(DataFrameLeft,font=("arial",12,"bold"),text=" PrecWarning:",padx=2,pady=6)
        ...
        txtProductQt=Entry(DataFrameLeft,textvariable=self.product_var,font=("arial",13,"bold"),bg="white",bd=2,relief=RIDGE,width=29)
     ...
     #===================== DataFrame Right========================
        ...
        txtmedName=Entry(DataFrameRight,textvariable=self.addmed_var,font=("arial",13,"bold"),bg="white",bd=2,relief=RIDGE,width=29)
        ...
        Table_frame.place(x=0,y=1,width=1500,height=250)
        ...
        self.pharmacy_table.heading("reg",text="Refrence No")
    ...
    def add_data(self):
        if self.ref_var.get()==""or self.lot_var.get()=="":
            ...
            conn=mysql.connector.connect(host="localhost",username="root",password="PHW#84#jeor",database="mydata")
            my_cursor=conn.cursor()
            my_cursor.execute("insert into pharmaacy values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(
                                                                                    self.ref_var.get(),
    ...
    def fetch_data(self):
        conn=mysql.connector.connect(host="localhost",username="root",password="PHW#84#jeor",database="mydata")
        my_cursor=conn.cursor()
        my_cursor.execute("select * from pharmaacy")
        row=my_cursor.fetchall()
        if len(row)!=0:
    ...
    def Update(self):
        if self.ref_var.get()=="" or self.lot_var.get()=="":
            ...
            conn=mysql.connector.connect(host="localhost",username="root",password="PHW#84#jeor",database="mydata")
            my_cursor=conn.cursor()
            my_cursor.execute("update pharmaacy set cmpName=%s,Type=%s,medname=%s,lot=%s,issuedate=%s,expdate=%s,uses=%s,sideeffect=%s,warning=%s,dosage=%s,price=%s,product=%s  where refno=%s",(
    ...
    def delete(self):
        ...
        sql="delete from pharmaacy where refno=%s"
    ...
    def search_data(self):
        conn = mysql.connector.connect(host="localhost", username="root", password="PHW#84#jeor", database="mydata")
        my_cursor = conn.cursor()
        my_cursor.execute("SELECT * FROM pharmaacy WHERE " + str(self.search_var.get()) + " LIKE '%" + str(self.searchTxt_var.get()) + "%'")
